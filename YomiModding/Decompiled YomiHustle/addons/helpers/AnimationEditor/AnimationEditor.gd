...
func save_to_state():
	...
	for frame in positions:
		if not frame in loaded_object.throw_positions:
...
func load_animation_data(node: ObjectState, animation = null):
	...
	anim_name = get_anim_name(node) if not animation else animation
