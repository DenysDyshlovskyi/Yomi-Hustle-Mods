...
func update_selected_move(move_state):
	...
	juke_dir.set_S( not fighter.is_grounded())
	juke_dir.set_SW( not fighter.is_grounded())
	juke_dir.set_SE( not fighter.is_grounded())

	juke_dir.set_N(( not fighter.is_grounded() and fighter.air_movements_left > 0) or air_juke)
	juke_dir.set_NE(( not fighter.is_grounded() and fighter.air_movements_left > 0) or air_juke)
	juke_dir.set_NW(( not fighter.is_grounded() and fighter.air_movements_left > 0) or air_juke)
